...
def repo_check(Repo, Directory, metrics, repeats, goal, month):
    data = data_github_monthly(Repo, Directory, goal)

    for way in range(7,8):
...
if __name__ == '__main__':
    ...
    path = r'../data/zephyr/'
    repo = "zephyr_new_contributor.csv"

    # for i in range(7):
    #     repo_check(repo, path, metrics=0, repeats=1, goal=i, month=1)

    repo_check(repo, path, metrics=0, repeats=1, goal=7, month=1)
